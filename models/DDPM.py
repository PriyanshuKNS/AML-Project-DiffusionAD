...
## ================================================================================================================================
    ...
    #     return loss,pred_x_0_norm_guided,normal_t,x_normal_t,x_noisier_t
...
## ================================================================================================================================
    ...
    # VERSION 1 (Given in the paper) 
    # Using x_0_b to get a better value of x_0_s
    
    def norm_guided_one_step_denoising(self, model, x_0, anomaly_label,args, t_s = -1, t_b = -1):
        if t_s == -1:
            t_s = torch.randint(0, args["less_t_range"], (x_0.shape[0],),device=x_0.device)
        if t_b == -1:    
            t_b = torch.randint(args["less_t_range"],self.num_timesteps,(x_0.shape[0],),device=x_0.device)
        
        loss_s, x_s, eps_s = self.calc_loss(model, x_0, t_s)
        loss_b, x_b, eps_b = self.calc_loss(model, x_0, t_b)
        
        x_0_b = self.predict_x_0_from_eps(x_b, t_b, eps_b).clamp(-1, 1)     # n := x_0_b
        n = x_0_b
        n_s = self.sample_q(n, t_s, eps_s)   

        loss = None 
        if anomaly_label != None:
            loss = (loss_s["loss"]+loss_b["loss"])[anomaly_label==0].mean()
            if torch.isnan(loss):
                loss.fill_(0.0)

        eps_s_new = eps_s - extract(self.sqrt_one_minus_alphas_cumprod, t_s, x_s.shape, x_0.device) * args["condition_w"] * (n_s - x_s)
        x_0_g = self.predict_x_0_from_eps(x_s, t_s, eps_s_new).clamp(-1, 1)

        return loss, x_0_g 

    def norm_guided_one_step_denoising_eval(self, model, x_0, t_s, t_b, args):
        loss, x_0_g = self.norm_guided_one_step_denoising(model, x_0, None, args, t_s, t_b)
        return x_0_g   
    
## ================================================================================================================================
  
    # VERSION 2
    # Using x_0_s to get better value of x_0_b (Opposite of what's done in the paper)
    def norm_guided_one_step_denoising2(self, model, x_0, anomaly_label,args, t_s = -1, t_b = -1):
        if t_s == -1:
            t_s = torch.randint(0, args["less_t_range"], (x_0.shape[0],),device=x_0.device)
        if t_b == -1:    
            t_b = torch.randint(args["less_t_range"],self.num_timesteps,(x_0.shape[0],),device=x_0.device)
        ...
        x_0_s = self.predict_x_0_from_eps(x_s, t_s, eps_s).clamp(-1, 1)     # n := x_0_s
        n = x_0_s
        n_b = self.sample_q(n, t_b, eps_b)   
        ...
        eps_b_new = eps_b - extract(self.sqrt_one_minus_alphas_cumprod, t_b, x_b.shape, x_0.device) * args["condition_w"] * (n_b - x_b)
        x_0_g = self.predict_x_0_from_eps(x_b, t_b, eps_b_new).clamp(-1, 1)

        return loss, x_0_g
    
    def norm_guided_one_step_denoising_eval2(self, model, x_0, t_s, t_b, args):
        loss, x_0_g = self.norm_guided_one_step_denoising2(model, x_0, None, args, t_s, t_b)
        return x_0_g
    
## ================================================================================================================================

    # VERSION 3
    # Sample tau (the boundary between t_s and t_b) randomly in a given range
    def norm_guided_one_step_denoising3(self, model, x_0, anomaly_label,args, t_s = -1, t_b = -1):
        
        var = 0.2 
        tau = args["less_t_range"]
        TT = int(random.uniform(tau - tau*var, tau + tau*var))   
        # print("TT: ", TT) 
        
        
        if t_s == -1:
            t_s = torch.randint(0, TT, (x_0.shape[0],),device=x_0.device)
        if t_b == -1:    
            t_b = torch.randint(TT,self.num_timesteps,(x_0.shape[0],),device=x_0.device)
    ...
    def norm_guided_one_step_denoising_eval3(self, model, x_0, t_s, t_b, args):
        loss, x_0_g = self.norm_guided_one_step_denoising3(model, x_0, None, args, t_s, t_b)
        return x_0_g
...
## ================================================================================================================================
    ...
    # VERSION 4
    # Simplified form : Just choose the better of the two using some metric, i.e. x_0_s and x_0_b and see how it performs
    def norm_guided_one_step_denoising4(self, model, x_0, anomaly_label,args, t_s = -1, t_b = -1):
        if t_s == -1:
            t_s = torch.randint(0, args["less_t_range"], (x_0.shape[0],),device=x_0.device)
        if t_b == -1:    
            t_b = torch.randint(args["less_t_range"],self.num_timesteps,(x_0.shape[0],),device=x_0.device)
        ...
        x_0_b = self.predict_x_0_from_eps(x_b, t_b, eps_b).clamp(-1, 1)    
        x_0_s = self.predict_x_0_from_eps(x_s, t_s, eps_s).clamp(-1, 1)   
        ...
        return loss, x_0_s, x_0_b 

    def norm_guided_one_step_denoising_eval4(self, model, x_0, t_s, t_b, args):
        loss, x_0_s, x_0_b = self.norm_guided_one_step_denoising4(model, x_0, None, args, t_s, t_b)
        return x_0_s, x_0_b   
