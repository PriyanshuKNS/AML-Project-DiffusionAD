...
def train(training_dataset_loader, testing_dataset_loader, args, data_len,sub_class,class_type,device, version ):
    ...
    for epoch in tqdm_epoch:
        ...
        for i, sample in enumerate(tbar):
            ...
            if version == 1:
                noise_loss, pred_x0  = ddpm_sample.norm_guided_one_step_denoising(unet_model, aug_image, anomaly_label,args)
            elif version == 2:
                noise_loss, pred_x0  = ddpm_sample.norm_guided_one_step_denoising2(unet_model, aug_image, anomaly_label,args)
            elif version == 3:
                noise_loss, pred_x0  = ddpm_sample.norm_guided_one_step_denoising3(unet_model, aug_image, anomaly_label,args)
            
            
            
            
            if version == 4:
                noise_loss, pred_1, pred_2 = ddpm_sample.norm_guided_one_step_denoising4(unet_model, aug_image, anomaly_label,args)
                
                pred_mask1 = seg_model(torch.cat((aug_image, pred_1), dim=1)) 
                focal_loss1 = loss_focal(pred_mask1,anomaly_mask)
                smL1_loss1 = loss_smL1(pred_mask1, anomaly_mask)
                loss1 = noise_loss + 5*focal_loss1 + smL1_loss1
                
                pred_mask2 = seg_model(torch.cat((aug_image, pred_2), dim=1)) 
                focal_loss2 = loss_focal(pred_mask2,anomaly_mask)
                smL1_loss2 = loss_smL1(pred_mask2, anomaly_mask)
                loss2 = noise_loss + 5*focal_loss2 + smL1_loss2
                
                if loss1 < loss2:
                    pred_mask = pred_mask1 
                    focal_loss = focal_loss1 
                    smL1_loss = smL1_loss1 
                    loss = loss1
                else:
                    pred_mask = pred_mask2 
                    focal_loss = focal_loss2 
                    smL1_loss = smL1_loss2
                    loss = loss2
        ...
        if (epoch+1) % 50==0:
            temp_image_auroc,temp_pixel_auroc= eval(testing_dataset_loader,args,unet_model,seg_model,data_len,sub_class,device, version)
            ...
            if(temp_image_auroc+temp_pixel_auroc>=best_image_auroc+best_pixel_auroc):
                if temp_image_auroc>=best_image_auroc:
                    save(version, unet_model,seg_model, args=args,final='best',epoch=epoch,sub_class=sub_class)
                    best_image_auroc = temp_image_auroc
                    best_pixel_auroc = temp_pixel_auroc
                    best_epoch = epoch
                
            
    save(version, unet_model,seg_model, args=args,final='last',epoch=args['EPOCHS'],sub_class=sub_class)
...
def eval(testing_dataset_loader,args,unet_model,seg_model,data_len,sub_class,device, version):
    unet_model.eval()
    seg_model.eval()
    ...
    for i, sample in enumerate(tbar):
        ...
        if version == 1:
            # print("train.py: Inside function eval: version: ", version)
            pred_x_0_condition = ddpm_sample.norm_guided_one_step_denoising_eval(unet_model, image, normal_t_tensor,noiser_t_tensor,args)
        elif version == 2:
            pred_x_0_condition = ddpm_sample.norm_guided_one_step_denoising_eval2(unet_model, image, normal_t_tensor,noiser_t_tensor,args)
        elif version == 3:
            pred_x_0_condition = ddpm_sample.norm_guided_one_step_denoising_eval3(unet_model, image, normal_t_tensor,noiser_t_tensor,args)
        elif version == 4:
            
            pred_1, pred_2 = ddpm_sample.norm_guided_one_step_denoising_eval4(unet_model, image, normal_t_tensor,noiser_t_tensor,args)
            
            pred_mask1 = seg_model(torch.cat((image, pred_1), dim=1)) 
            out_mask1 = pred_mask1
            topk_out_mask1 = torch.flatten(out_mask1[0], start_dim=1)
            topk_out_mask1 = torch.topk(topk_out_mask1, 50, dim=1, largest=True)[0]
            image_score1 = torch.mean(topk_out_mask1)
            
            pred_mask2 = seg_model(torch.cat((image, pred_2), dim=1)) 
            out_mask2 = pred_mask2
            topk_out_mask2 = torch.flatten(out_mask2[0], start_dim=1)
            topk_out_mask2 = torch.topk(topk_out_mask2, 50, dim=1, largest=True)[0]
            image_score2 = torch.mean(topk_out_mask2)
            
            image_score = None
            out_mask = None
            if image_score1 > image_score2:
                image_score = image_score1 
                out_mask = out_mask1
            else:
                image_score = image_score2
                out_mask = out_mask2
...
def save(version, unet_model,seg_model, args,final,epoch,sub_class):
        ...
        torch.save(
            {
                ...
                "args":                 args
                }, f'{args["output_path"]}/model/diff-params-ARGS={args["arg_num"]}/{sub_class}/params-{final}-{version}.pt'
            )
...
def main():
    ...
    for sub_class in current_classes:    
        ...
        train(training_dataset_loader, test_loader, args, data_len,sub_class,class_type,device, version)
