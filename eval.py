...
def load_checkpoint(param, device,sub_class,checkpoint_type,args, version):

    ck_path = f'{args["output_path"]}/model/diff-params-ARGS={param}/{sub_class}/params-{checkpoint_type}-{version}.pt'
...
def load_parameters(device,sub_class,checkpoint_type, version):
    ...
    output = load_checkpoint(param[4:-5], device,sub_class,checkpoint_type,args, version)
...
def testing(testing_dataset_loader, args,unet_model,seg_model,data_len,sub_class,class_type,checkpoint_type,device, version):
    ...
    for i, sample in enumerate(tbar):
        ...
        if version == 1:
            ...
            # pred_matrix_pixel.extend(out_mask1[0].detach().cpu().numpy())
            ...
            # gt_matrix_pixel.extend(gt_mask[0].detach().cpu().numpy().astype(int))
            # pred_matrix_pixel.extend(out_mask2[0].detach().cpu().numpy())
...
def main():
    ...
    for sub_class in current_classes:
        args, output = load_parameters(device,sub_class,checkpoint_type, version)
        ...
        testing(test_loader, args,unet_model,seg_model,data_len,sub_class,class_type,checkpoint_type,device, version)
